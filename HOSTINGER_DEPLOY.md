# Deploy LearningBanyan on Hostinger
...
Repo: `https://github.com/learningbanyan/LearningBanyan.git`
...
git clone --branch "$BRANCH" "$REPO_URL" /var/www/learningbanyan
...
- clone/update the repo into `/var/www/learningbanyan`
...
- enable systemd `learningbanyan` (Gunicorn) and Nginx

Create an admin user:

```bash
cd /var/www/learningbanyan
...
Then in `/var/www/learningbanyan/.env` set:
...
```bash
systemctl restart learningbanyan
```
...
export VPS_IP=YOUR_VPS_IP
export SKIP_NGINX=1
git clone https://github.com/learningbanyan/LearningBanyan.git /var/www/learningbanyan
bash /var/www/learningbanyan/deploy/hostinger/setup.sh
chown -R nobody:nogroup /var/www/learningbanyan
```
...
```bash
/var/www/learningbanyan/venv/bin/python -c "import sys; print(sys.version)"
```

## Environment variables (server `.env`)

Never commit `.env`. On the VPS it lives at `/var/www/learningbanyan/.env`.
...
| `DATABASE_URL` | `mysql://user:pass@127.0.0.1:3306/learningbanyan` or Postgres URL |
...
| 502 Bad Gateway | `systemctl status learningbanyan` and `journalctl -u learningbanyan -n 80` |
| Static/CSS missing | `python manage.py collectstatic --noinput`; confirm Nginx `/static/` alias |
| Media uploads 404 | `chown -R www-data:www-data /var/www/learningbanyan/media` (Nginx) or `nobody:nogroup` (OLS) |
