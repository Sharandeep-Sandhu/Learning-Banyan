#!/usr/bin/env bash
# Hostinger VPS bootstrap for LearningBanyan (Ubuntu 24.04/26.04, Python 3.12+).
#
...
#
# Optional:
#   export APP_DIR=/var/www/learningbanyan
...
DOMAIN="${DOMAIN:-}"
VPS_IP="${VPS_IP:-}"
REPO_URL="${REPO_URL:-https://github.com/learningbanyan/LearningBanyan.git}"
APP_DIR="${APP_DIR:-/var/www/learningbanyan}"
...
if [ -z "$DOMAIN" ]; then
  echo "Set DOMAIN first, e.g. export DOMAIN=learningbanyan.com"
  exit 1
fi
...
DB_NAME="${DB_NAME:-learningbanyan}"
DB_USER="${DB_USER:-learningbanyan}"
...
if [ "$SKIP_NGINX" != "1" ]; then
  ...
  sed "s|/var/www/learningbanyan|${APP_DIR}|g" "$SERVICE_SRC" > /etc/systemd/system/learningbanyan.service
  sed -e "s|SITE_DOMAIN|${DOMAIN}|g" \
      -e "s|VPS_IP|${VPS_IP}|g" \
      -e "s|/var/www/learningbanyan|${APP_DIR}|g" \
      "$NGINX_SRC" > /etc/nginx/sites-available/learningbanyan
  ln -sfn /etc/nginx/sites-available/learningbanyan /etc/nginx/sites-enabled/learningbanyan
  rm -f /etc/nginx/sites-enabled/default
  nginx -t
  systemctl daemon-reload
  systemctl enable --now learningbanyan
  ...
  echo "  systemctl restart learningbanyan"
else
